...
\begin{document}	%设置课程标题
	...
	%设置实验人信息以及实验时间表格
		...
		\begin{center}
			\begin{tabular}{lcr}
				...
				{\songti  学院：计算机学院 \quad 时间：2025年3月11日 \quad 组别: L组11号}\\
	...
	\end{equation}
	 ...
	 \subsection*{[实验步骤]}
  	...
  	\par 取两个滑块分别放在电子天平上，测得两滑块的质量分别为$m_1=131.82g$,$m_2=131.85g$,误差$\Delta m = 0.03g<0.1g$。打开气泵和数字毫秒计，等待一段时间后将滑块1放在导轨上并给予初速度，观察数字毫秒计上显示的滑块通过两次光电门的时间，若两次的时间差大于$0.1ms$,则保持旋钮$Q$不动，调节旋钮$P$，重复试验直到两者差值小于$0.1ms$为止。本次试验动态调平时时间分别为$78.06ms$,$78.14ms$。
  		\begin{figure}[h]
  		\centering
  	...
  	\par 调节两个滑块的方向，使得滑块上的弹簧同时向内，同时调节遮光条的方向使得其能被光电门检测到。将两个滑块和光电门按照图1的位置关系摆放。用手保持右边的滑块不动，给予左侧的滑块初速度，记录下左侧滑块第一次通过左侧光电门和碰撞后右边滑块第一次通过右边光电门的时间，重复六次，结果记录在数据处理中。
  	\subsubsection*{动能损失量的测定}
  	\par 和动量定理步骤大致一样，不同点在于将两个滑块调换位置，使得两滑块尾部的魔术贴向内，碰撞后两个滑块会一起运动，且记录的是左侧滑块两次通过光电门的时间(即数字毫秒计第一次和第三次的记录)，重复六次，结果记录在数据处理中。
  	...
  	\end{figure}
  \subsection*{[数据处理]}
\[\Delta s_1=0.04970m \quad \Delta s_2=0.04970m \quad \Delta s_1^{'}=0.04970m \]
	\par 为慎重起见，我们规定：乘除运算的结果应比参与运算分量中有效数字位数最少的测得值多取一位，简记为“多取一位”的法则。故速度、恢复系数、动量百分差、动能百分差取五位有效数字。根据$v = \dfrac{s}{t}$可得
...
& \multicolumn{1}{l|}{$\Delta t_1/s$}     & \multicolumn{1}{l|}{$u/(m\cdot s^{-1})$} & \multicolumn{1}{l|}{$\Delta t_2/s$}     & $v/(m\cdot s^{-1})$ & \multicolumn{1}{l|}{$\Delta t_1^{'}/s$}     & \multicolumn{1}{l|}{$u^{'}/(m\cdot s^{-1})$} & \multicolumn{1}{l|}{$\Delta t_2^{'}/s$}     & $u^{'}/(m\cdot s^{-1})$ \\ \hline
			1                    & \multicolumn{1}{l|}{0.10992} & \multicolumn{1}{l|}{0.45215} & \multicolumn{1}{l|}{0.11325} & 0.43885 & \multicolumn{1}{l|}{0.08822} & \multicolumn{1}{l|}{0.56336} & \multicolumn{1}{l|}{0.18827} & 0.26398 \\ \hline
			2                    & \multicolumn{1}{l|}{0.10960} & \multicolumn{1}{l|}{0.45347} & \multicolumn{1}{l|}{0.11258} & 0.44146 & \multicolumn{1}{l|}{0.08831} & \multicolumn{1}{l|}{0.56279} & \multicolumn{1}{l|}{0.18574} & 0.26758 \\ \hline
			3                    & \multicolumn{1}{l|}{0.10932} & \multicolumn{1}{l|}{0.45463} & \multicolumn{1}{l|}{0.11242} & 0.44209 & \multicolumn{1}{l|}{0.09098} & \multicolumn{1}{l|}{0.54627} & \multicolumn{1}{l|}{0.19811} & 0.25087 \\ \hline
		\end{tabular}
        \end{adjustbox}
\end{table}


\par 为更清晰地表达数值之间的差异程度，提供恢复系数、动量百分差、动能百分差保留两位有效数字版本
\par 完全弹性碰撞：
\par 恢复系数 \(e = \left|\frac{v_1 - v_2}{u_1 - u_2}\right| = 0.97351 \approx 0.97\)
\par 动量百分差 \(E_1 = \left|\frac{p_2 - p_1}{p_1}\right|=\left|\frac{m_2\Delta s_2\Delta t_1}{m_1\Delta s_1\Delta t_2}-1\right| = 0.027190 \approx 0.027\)
\par 动能百分差 \(E_2=\left|\frac{E_{k2}-E_{k1}}{E_{k1}}\right|=\left|\frac{m_2\Delta s_2^2\Delta t_1^2}{m_1\Delta s_1^2\Delta t_2^2}-1\right| = 0.055118 \approx 0.055\)

\par 完全非弹性碰撞：
\par 恢复系数 \(e = \left|\frac{v_1 - v_2}{u_1 - u_2}\right| = 0\)
\par 动量百分差 \(E_1'=\left|\frac{p_1'-p_2'}{p_1'}\right|=\left|\left(1 + \frac{m_2}{m_1}\right)\frac{\Delta t_1'}{\Delta t_2'}-1\right| = 0.058120 \approx 0.058\)
\par 动能百分差 \(E_2'=\left|\frac{E_{k2}'-E_{k1}'}{E_{k1}'}\right|=\left|\left(1 + \frac{m_2}{m_1}\right)\left(\frac{\Delta t_1'}{\Delta t_2'}\right)^2 - 1\right| = 0.55648 \approx 0.56\)
\par 动能损失百分误差 \(E_{\Delta}=\left|2\left(1 + \frac{m_2}{m_1}\right)\left(\frac{\Delta t_1'}{\Delta t_2'}\right)^2 - 1\right| = 11\% \) (保留两位有效数字)
...
\par 1. 测量工具的精度：
使用的数字毫秒计、游标卡尺、电子天平本身的精度限制是误差的一个来源。
\par 2. 碰撞并非理想情况：
理论计算假设碰撞要么是完全弹性的，要么是完全非弹性的。然而在现实中，碰撞后两个滑块的弹簧会缠绕在一起。这是因为两个滑块未发生对心碰撞，即便经过调整，也难以保证两个滑块的质心始终处于运动路径的直线上，从而导致实验误差。 
\par 3. 人为操作误差：
释放滑块时可能带有初速度，或者动态调平不够充分。
\par 4. 系统误差：
尽管气垫导轨能减小摩擦力，但摩擦力和空气阻力的影响依然存在。
