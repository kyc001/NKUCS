	...
	%设置实验人信息以及实验时间表格
		...
		\begin{center}
			\begin{tabular}{lcr}
				...
				{\songti  学院：计算机学院 \quad 时间：2025年4月8日 \quad 组别: L组11号}\\
...
\par 电桥灵敏度$S$是指:
	...
	\subsection*{[数据处理]}
	\subsubsection*{测量未知电阻$R_1$及灵敏度}
			\par 根据情况，选择$R_a=10\Omega$,$R_b=1000.0\Omega$，比例臂的倍数$C=0.01$。
		\begin{table}[!htbp]
		...
		\begin{tabular}{cccccc}
			...
			\midrule
			数据记录& $5003.0\Omega$&  $50.03\Omega$& $4\Omega$&$10.8nA$ & $13508.1nA$\\
			\bottomrule		
		\end{tabular}
	\end{table}
	
	\par 计算得：
	\[\rho_1=\sqrt{0.002^2+0.001^2+\left(\frac{0.1}{13508.1}\right)^2}\approx0.0022\]
	\[ \Delta R_1 = \rho_1\cdot R_1^{\prime}\approx 0.11\Omega\]
	\[ R_1 = R_1^{\prime}\pm \Delta R_1=(50.03\pm  0.11) \Omega\]
	...
	\begin{table}[!htbp]
		...
		\begin{tabular}{cccccc}
			\toprule
			\makebox[0.18\textwidth][c]{电桥状态} & \makebox[0.13\textwidth][c]{$R_0$}& \makebox[0.13\textwidth][c]{$R_2$}& \makebox[0.13\textwidth][c]{$\Delta R_0$}& \makebox[0.13\textwidth][c]{$\Delta I$}& \makebox[0.13\textwidth][c]{$S_1$}\\
			\midrule
			换臂前& $1184.1\Omega$&  $1184.1\Omega$& $1\Omega$&$16.7nA$ & $19774.47nA$\\
			换臂后& $1183.9\Omega$&  $1183.9\Omega$& $1\Omega$&$16.8nA$ & $19889.52nA$\\
		\bottomrule		
		\end{tabular}
	\end{table}
	...
	\par 利用换臂前后的数据计算：
	\[R_x\approx \frac{1}{2}(R_0^{\prime}+R_0^{\prime\prime})=1184.0\Omega\]
	\[\rho_x=\sqrt{\rho_0^2+\left(\frac{\delta}{S}\right)^2}=\sqrt{0.001^2+\left(\frac{0.1}{19774.47}\right)^2}\approx0.0010\]
	\[ \Delta R_x = \rho_x\cdot R_{x}^{\prime}\approx 1.2\Omega\]
	\par 从而得到:
	\[ R_x = R_{x}^{\prime}\pm \Delta R_x=(1184.0\pm 1.2) \Omega\]
	...
	\par 取$R_a=R_b=100\Omega$,$R_0=1200\Omega$
		...
		\begin{tabular}{cccccccc}
			\toprule
			\makebox[0.20\textwidth][c]{电源电压$(V)$} & \makebox[0.08\textwidth][c]{$0.50$}& \makebox[0.08\textwidth][c]{$1.04$}& \makebox[0.08\textwidth][c]{$1.51$}& \makebox[0.08\textwidth][c]{$2.01$}& \makebox[0.08\textwidth][c]{$2.54$}&\makebox[0.08\textwidth][c]{$3.01$}&\makebox[0.08\textwidth][c]{$3.50$}\\
			\midrule
			$\Delta R_0(\Omega)$& $2$&  $2$& $2$&$2$ & $2$&$2$&$2$\\
			$\Delta I(nA)$& $5.9$&$12.0$&  $17.6$& $23.6$&$29.9$ & $35.8$&$ 41.8$\\
			$S(*10^3)$      &3.540&7.200&10.56&14.16&17.94&21.48&25.08\\
			\bottomrule		
		\end{tabular}
	\end{table}
\clearpage
\par 画出图像：
	\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{figures/S-E图.png}
	\caption{\footnotesize $S-E$图}
\end{figure}
